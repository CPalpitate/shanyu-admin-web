...
/**
 * 获取单服务配置
 * @param env 环境变量对象，从vite.config.ts传入
 * @returns 对应环境的服务配置
 */
export const getSingleServiceConfig = (env: ImportMetaEnv): Record<ServiceEnvType, SingleServiceConfig> => {
    const {VITE_API_URL} = env

    return {
        // 开发环境接口配置
        dev: {
            apiUrl: VITE_API_URL || 'http://localhost:8911',
        },

        // 测试环境接口配置
        test: {
            apiUrl: VITE_API_URL || 'https://test-api.example.com',
        },

        // 生产环境接口配置
        prod: {
            apiUrl: VITE_API_URL || 'https://api.example.com',
        },
    }
}
/**
