...
export const authApi = {
     ...
     * @returns 新的访问令牌
     */
    refreshToken(refreshToken: string): Promise<T.R<{ accessToken: string; expiresIn: number }>> {
     ...
     * @returns 验证结果
     */
    verifyToken(): Promise<T.R<boolean>> {
     ...
     * @returns 权限列表
     */
    getUserPermissions(): Promise<T.R<string[]>> {
     ...
     * @returns 验证码信息
     */
    getCaptcha(): Promise<T.R<T.CaptchaResponse>> {
        return request.get<T.CaptchaResponse>('/auth/captcha')
    },
     ...
     * @returns 发送结果
     */
    sendSmsCode(phone: string): Promise<T.R<boolean>> {
     ...
     * @returns 验证结果
     */
    verifySmsCode(phone: string, smsCode: string): Promise<T.R<boolean>> {
     ...
     * @returns 检查结果
     */
    checkUsername(username: string): Promise<T.R<boolean>> {
     ...
     * @returns 检查结果
     */
    checkEmail(email: string): Promise<T.R<boolean>> {
     ...
     * @returns 检查结果
     */
    checkPhone(phone: string): Promise<T.R<boolean>> {
